...
- **Wine Project — modifica dei testi e filtro reazioni** (`/impostazioni/wine-project`): ogni email della sequenza ha un pulsante di salvataggio proprio (`PATCH /api/wine-project/automation` con `sequence`), oltre al salvataggio completo in fondo alla pagina. La sezione "Chi ha reagito" elenca le cantine filtrate per reazione (`GET /api/wine-project/contacts?engagement=opened|clicked|silent|all`) con la colonna **Stato**: aperture e click da soli facevano sembrare da lavorare anche chi aveva già detto no, quindi la riga dice se quell'**indirizzo** ha risposto (`replied`, cercato su `gmail_messages.from_email` come il blocco della sequenza); lo stesso filtro esiste in `/contacts` ("Email: qualsiasi reazione", client-side su `email_open_count` / `email_click_count`)
...
- **Scansione della posta in arrivo** (`sweepInboundGmailReplies` in `src/lib/server/gmail.ts`, `POST /api/automation/inbound-replies`): fino a qui ogni rilevamento di risposta partiva **dal contatto** — `reply-monitor` guarda chi è stato scritto di recente, `wine-project-replies` ruota sugli arruolati — quindi chi non entrava nella rotazione restava invisibile e la sequenza continuava a scrivere a chi aveva già risposto «non siamo interessati». Questo giro parte dalla casella: una sola ricerca Gmail sugli ultimi `days` giorni, i mittenti riconosciuti nel CRM (per indirizzo, quindi **tutte** le schede gemelle) passano a `syncContactGmailMessages`, che è già il posto dove una risposta diventa attività, promozione da holding, classificazione AI, stato del contatto e stop della sequenza Wine. Solo i metadati vengono letti per i mittenti sconosciuti; una scheda che ha già in archivio tutti i messaggi del proprio mittente non viene risincronizzata. Gira in testa a `05-reply-monitor` e `12-wine-project-automation` (ogni 30 min) e dal pulsante "Controlla risposte" in `/impostazioni/wine-project`
- **Ultimo cancello prima dell'invio Wine**: `/api/automation/wine-project-campaigns` ricontrolla `wineSequenceBlockReason` sull'indirizzo appena prima di consegnare il gruppo ad Acumbamail. Il filtro `closed()` guarda solo la riga in esame, e fra la messa in coda e l'invio può passare mezzo giorno: è lì che una risposta arrivata nel frattempo veniva ignorata. Il motivo vero finisce in `skip_reason`
...
- `05-reply-monitor.json` — inbox sweep (`/api/automation/inbound-replies`), then Gmail reply sync + AI classification, then draft reconciliation (every 30 min)
...
- `12-wine-project-automation.json` — inbox sweep, Wine Project follow-ups, campaign groups, engagement and replies (every 30 min)
