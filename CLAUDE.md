...
- **Wine Project — una scheda compilata e un lead, anche senza demo**: `POST /api/speaqi/leads` accetta `wine_form_submitted` (scheda compilata) oltre a `wine_demo_contact` (demo pronta). Contavano solo le demo, e l'analisi che finisce in `ANALYSIS_FAILED` o `STAFF_REVIEW_REQUIRED` non produce demo: quei lead non entravano proprio. Il contatto si riconosce in tre modi, in ordine di certezza — `campaign_token` firmato, email, dominio di `source_url` (le schede wine non hanno `normalized_website`, quindi il confronto e sul dominio dell'email) — cosi un form senza email non finisce piu in un 400. `attempt_id` rende il tentativo idempotente: form e demo dello stesso tentativo contano una volta sola. I riquadri di `/impostazioni/wine-project` contano **cantine**, non righe di attivita, con la stessa query della lista che aprono (`src/lib/server/wine-project-engagement.ts`): il conteggio vecchio tirava giu' tutte le attivita in una query sola e PostgREST tronca a 1000 righe, quindi bastavano abbastanza click sulla landing per far sparire i form dal totale
- **Wine Project — riconciliazione delle schede generate**: il webhook e un avviso, e un avviso che non parte non lascia traccia. Siccome ogni demo nasce da un'email che sta nel CRM, una scheda generata ha sempre una cantina a cui appartenere: la discrepanza fra i due conti e un errore, non una sfumatura. `src/lib/server/wine-project-demo-reconcile.ts` prende l'elenco delle analisi e registra quelle mancanti — stesse attivita, stessa sequenza fermata, stessa chiamata in coda del webhook, ma con `created_at` retrodatato e `metadata.reconciled = true`. Aggancio in quattro strade, dalla piu certa: email, dominio del sito, e **click sulla landing** entro 90 minuti prima dell'analisi (il link porta il codice firmato, quindi il click identifica la cantina anche quando la sua email sta su un altro dominio: `alfeu.it` contro `alfeuwinery@gmail.com`) — e solo se in quella finestra ha cliccato una cantina sola, perche attaccare la scheda alla cantina sbagliata e peggio che lasciarla scoperta. Superfici: `POST /api/wine-project/reconcile-demos` (sessione, admin; `dry_run` mostra l'esito senza scrivere) dietro il riquadro "Schede generate sull'app Speaqi" in `/impostazioni/wine-project`, e `POST /api/automation/wine-project-demos` (`AUTOMATION_SECRET`) per l'app o un cron. `parseDemoList` legge la tabella dell'app incollata cosi com'e: tiene sito ed email, ignora etichette e conteggi
