...
| `STRIPE_VIDEO_MAP_PRICE_ID` / `STRIPE_VIDEO_MAP_COUPON_ID` | Prodotto "Video nella mappa" del catalogo Stripe (488 €/anno IVA inclusa) + coupon permanente (122 €). Opzionali: senza, il checkout scrive il prezzo al volo |
...
- **Catalogo Stripe**: con `STRIPE_VIDEO_MAP_PRICE_ID` e `STRIPE_VIDEO_MAP_COUPON_ID` il checkout usa il prodotto del catalogo (prezzo pieno 488 € + coupon 122 € **per sempre**, cosi' anche i rinnovi restano a 366 €) invece di `price_data`. Si usa solo se `catalogAmountCents` coincide al centesimo con il totale firmato; altrimenti (preventivo con un importo diverso, coupon non "per sempre", prezzo non annuale) si torna al prezzo scritto al volo. La metadata `pricing=catalog|inline` dice quale strada e' stata presa; una sessione aperta con l'altra strada viene chiusa (`expire`) prima di crearne una nuova
...
```bash
npm run test:unit   # motore campagne, Wine Project, WhatsApp, lavagna To Do, Telegram, abbonamenti/firma/link vendita e programma commerciali
