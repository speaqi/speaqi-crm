...
/**
 * Da "+39 333 1234567" a "393331234567@c.us". OpenWA vuole il WID completo:
 * numero internazionale senza + e senza spazi, con il suffisso del tipo di chat.
 */
...
export function isWhatsappNotifyEnabled() {
  return String(process.env.WHATSAPP_NOTIFY_ENABLED || '').trim().toLowerCase() === 'true'
}
...
export function whatsappConfig(): WhatsappConfig | null {
  ...
  const chatId = normalizeChatId(process.env.WHATSAPP_NOTIFY_TO)
  if (!baseUrl || !apiKey || !sessionId || !chatId) return null
  return { baseUrl, apiKey, sessionId, chatId }
}
...
export function whatsappConfigStatus() {
  ...
  if (!normalizeChatId(process.env.WHATSAPP_NOTIFY_TO)) missing.push('WHATSAPP_NOTIFY_TO')
  return {
    configured: missing.length === 0,
    enabled: isWhatsappNotifyEnabled(),
    missing,
    chat_id: normalizeChatId(process.env.WHATSAPP_NOTIFY_TO),
  }
}
...
export async function sendWhatsappText(
  ...
  if (!isWhatsappNotifyEnabled()) return { ok: false, error: 'Notifiche WhatsApp disattivate' }

  const chatId = normalizeChatId(options?.chatId) || config.chatId
